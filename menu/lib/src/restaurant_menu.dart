...
class RestaurantOrder extends _$RestaurantOrder {
  @override
  Future<RestaurantOrderInfo> build(int reservationId, [AuthType type = AuthType.user]) async {
    try {
      final response = await Dio().get(
          '${dotenv.env['${type.name.toUpperCase()}_API_URL']!}reservation-order-items',
  ...
  Future<void> selectCategory(int id) async {
    try {
      final response = await Dio().get(
          '${dotenv.env['${type.name.toUpperCase()}_API_URL']!}restaurant-category-items',
          queryParameters: {
  ...
  Future<bool> updateOrder(VoidCallback refreshFunction) async {
    try {
      await Dio().post('${dotenv.env['${type.name.toUpperCase()}_API_URL']!}update-order',
          data: {
