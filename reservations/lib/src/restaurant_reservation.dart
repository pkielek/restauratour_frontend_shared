...
class RestaurantReservation with _$RestaurantReservation {
  ...
  factory RestaurantReservation({
    ...
    String? tableId,
    @JsonKey(ignore: true) @Default(false) bool areDetailsEdited,
    @JsonKey(ignore: true) @Default("") String newDetails,
      ...
      for (final reservation in state.value!)
        if (reservation.id == id)
          reservation.copyWith(
              newDetails: reservation.additionalDetails, areDetailsEdited: true)
        else
          reservation
    ]);
  }

  void onEditDetails(int id, String? value) {
    if (value == null || value.length > 240) return;
    state = AsyncData([
      for (final reservation in state.value!)
        if (reservation.id == id)
          reservation.copyWith(newDetails: value)
        else
          reservation
    ]);
  }

  void cancelEditDetails(int id) {
    state = AsyncData([
      for (final reservation in state.value!)
        if (reservation.id == id)
          reservation.copyWith(newDetails: "", areDetailsEdited: false)
        else
          reservation
    ]);
  }

  Future<void> saveEditDetails(int id) async {
    try {
      await Dio().post(
          '${dotenv.env['USER_API_URL']!}update-reservation-details',
          data: {
            "reservation_id": id,
            "details": state.value!
      ...
      state = AsyncData([
        for (final reservation in state.value!)
          if (reservation.id == id)
            reservation.copyWith(
                additionalDetails: reservation.newDetails,
                newDetails: "",
                areDetailsEdited: false)
          else
            reservation
      ]);
    } on DioException catch (e) {
